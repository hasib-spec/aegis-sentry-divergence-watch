...
const OrbitsViewer = dynamic(() => import("@/components/OrbitsViewer"), {
  ...
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-[#030308]">
      <p className="font-mono text-[10px] text-zinc-700 animate-pulse">LOADING WEBGL...</p>
    </div>
  ),
});
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      switch (sortKey) {
        case "designation":
          return sortAsc ? a.designation.localeCompare(b.designation) : b.designation.localeCompare(a.designation);
        ...
        case "probabilityRatio": av = safeNum(a.probabilityRatio); bv = safeNum(b.probabilityRatio); break;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#030308]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black tracking-tight text-white leading-none">
                ...
                <span className="ml-2 text-xs font-normal text-zinc-500 tracking-wide">DIVERGENCE WATCH</span>
              </h1>
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1920px] mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col min-w-0 p-3 gap-2.5 overflow-y-auto max-h-[calc(100vh-52px)]">
          ...
          {loading && !data && (
            <div className="flex-1 flex items-center justify-center">
              <div className="font-mono text-[10px] space-y-1 text-zinc-600">
                ...
                <p className="text-orange-400 animate-pulse" style={{ animationDelay: "0.2s" }}>▸ ESA NEOCC/AEGIS UPLINK</p>
          ...
          {data && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="ml-auto text-[9px] font-mono text-zinc-700">{filtered.length} obj</span>
            </div>
          )}
                ...
                <table className="w-full">
                  <thead className="sticky top-0 bg-[#080810] z-10">
                    <tr className="border-b border-zinc-800/50 text-[8px] font-mono text-zinc-600 uppercase tracking-wider">
                      <th className="px-2.5 py-2 text-left cursor-pointer hover:text-white" onClick={() => handleSort("designation")}>
                        <span className="flex items-center gap-0.5">Object <SortIcon col="designation" /></span>
                      </th>
                      <th className="px-2.5 py-2 text-right">km</th>
                      <th className="px-2.5 py-2 text-right cursor-pointer hover:text-cyan-400" onClick={() => handleSort("nasaIp")}>
                        <span className="flex items-center gap-0.5 justify-end text-cyan-400/50">NASA IP <SortIcon col="nasaIp" /></span>
                      </th>
                      <th className="px-2.5 py-2 text-right cursor-pointer hover:text-orange-400" onClick={() => handleSort("esaIp")}>
                        <span className="flex items-center gap-0.5 justify-end text-orange-400/50">ESA IP <SortIcon col="esaIp" /></span>
                      </th>
                      <th className="px-2.5 py-2 text-right cursor-pointer hover:text-white" onClick={() => handleSort("absPalermoDelta")}>
                        <span className="flex items-center gap-0.5 justify-end">ΔPS <SortIcon col="absPalermoDelta" /></span>
                      </th>
                      <th className="px-2.5 py-2 text-right cursor-pointer hover:text-white" onClick={() => handleSort("probabilityRatio")}>
                        <span className="flex items-center gap-0.5 justify-end">Ratio <SortIcon col="probabilityRatio" /></span>
                      </th>
                    ...
                    {filtered.slice(0, 80).map((t, i) => {
                      ...
                      return (
                        <tr
                          ...
                          className={`border-b border-zinc-800/20 cursor-pointer transition-colors ${isSel ? "bg-cyan-500/[0.05]" : "hover:bg-zinc-800/20"}`}
                        >
                          <td className="px-2.5 py-1.5">
                            <span className="font-mono text-[11px] text-zinc-200">{t.designation}</span>
                            <span className="block text-[8px] text-zinc-700 truncate max-w-[120px]">{t.fullname}</span>
                          </td>
                          ...
                          <td className={`px-2.5 py-1.5 text-right font-mono text-[10px] ${Math.abs(safeNum(t.palermoDelta)) > 1 ? "text-red-400" : Math.abs(safeNum(t.palermoDelta)) > 0.3 ? "text-amber-400" : "text-emerald-400"}`}>
                            {safeNum(t.palermoDelta) !== 0 ? (safeNum(t.palermoDelta) > 0 ? "+" : "") + safeNum(t.palermoDelta).toFixed(3) : "—"}
                          </td>
                          <td className="px-2.5 py-1.5 text-right font-mono text-[10px] text-zinc-500">
                            {safeNum(t.probabilityRatio) > 0 && safeNum(t.probabilityRatio) < 900 ? safeNum(t.probabilityRatio).toFixed(2) + "×" : "—"}
                          </td>
                          <td className="px-2.5 py-1.5 text-center">
                            <span className={`text-[7px] font-mono px-1 py-0.5 rounded ${sevCls}`}>{sev.slice(0, 4)}</span>
                          </td>
                        </tr>
                      );
                    })}
          ...
          {selected && (
            <div className="rounded-lg border border-zinc-700/40 bg-[#080810]/80 p-3.5">
              <div className="flex items-center justify-between mb-2.5">
                ...
                <button onClick={() => setSelected(null)} className="text-zinc-700 hover:text-white text-[10px] font-mono">✕</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                <div className="rounded-md border border-cyan-500/10 bg-cyan-500/[0.02] p-2.5 space-y-1">
                  <p className="text-[8px] font-mono text-cyan-400 uppercase tracking-widest flex items-center gap-1"><Radio size={8} /> NASA SENTRY-II</p>
                  ...
                  <p className="text-[7px] font-mono text-cyan-400/40 pt-1 border-t border-cyan-500/5">{selected.nasa.hasNonGrav ? "✓ YARKOVSKY A1,A2" : "✗ NO NON-GRAV"}</p>
                </div>
                <div className="rounded-md border border-orange-500/10 bg-orange-500/[0.02] p-2.5 space-y-1">
                  <p className="text-[8px] font-mono text-orange-400 uppercase tracking-widest flex items-center gap-1"><Radio size={8} /> ESA NEOCC</p>
                  ...
                  <p className="text-[7px] font-mono text-orange-400/40 pt-1 border-t border-orange-500/5">✗ GRAV-ONLY</p>
                </div>
                <div className="rounded-md border border-red-500/10 bg-red-500/[0.02] p-2.5 space-y-1">
                  <p className="text-[8px] font-mono text-red-400 uppercase tracking-widest flex items-center gap-1"><AlertTriangle size={8} /> DIVERGENCE</p>
                  ...
                  <Row l="da/dt" v={(safeNum(selected.yarkovskyDriftAUDay) * 365.25e6).toExponential(2) + " AU/Myr"} />
                  <Row l="Severity" v={selected.divergenceSeverity} />
                  <div className="flex gap-2 pt-1 border-t border-red-500/5">
                    <a href={`https://ssd-api.jpl.nasa.gov/sentry.api?des=${encodeURIComponent(selected.designation)}`} target="_blank" rel="noopener noreferrer" className="text-[8px] font-mono text-cyan-400/50 hover:text-cyan-400 flex items-center gap-0.5"><ExternalLink size={7} /> NASA</a>
                    <a href={`https://neo.ssa.esa.int/PSDB-portlet/download?file=${encodeURIComponent(selected.designation)}.risk`} target="_blank" rel="noopener noreferrer" className="text-[8px] font-mono text-orange-400/50 hover:text-orange-400 flex items-center gap-0.5"><ExternalLink size={7} /> ESA</a>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* RIGHT: GLOBE */}
        {showGlobe && (
          <div className="w-full lg:w-[40%] h-[35vh] lg:h-[calc(100vh-52px)] border-l border-zinc-800/30 relative" style={{ minHeight: "300px" }}>
